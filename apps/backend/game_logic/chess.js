...
class Chess {
    ...
    for (let i = 0; i < piecesToPlace.length; i++) {
      ...
      if (moreThanOneOfPiece) {
        squaresToPlace.forEach(
          (square) => (this.getSquare(board, square).piece = piece)
        );
      } else {
  ...
  isSquareOccupied(board, square) {
    const squareIsOccupied =
      this.isSquareOnBoard(square) && this.getSquare(board, square).piece;
  ...
  isKingInCheck(board, kingColor) {
    ...
    const kingIsInCheck = unsafeSquares.some(
      (unsafeSquare) => unsafeSquare === kingsSquare
    );
    ...
    for (let y = 0; y < 8; y++) {
      for (let x = 0; x < 8; x++) {
        ...
        if (this.getPiecesColor(board, squareToCheck) === color) {
          ...
          const possibleSquares = this.findSquaresForPiece(
            board,
            piecesSquare,
            "possibleMoves"
          );
  ...
  findSquaresForKing(board, kingsSquare, squaresToFind) {
    ...
    for (const move in kingMoves) {
      ...
      if (squaresToFind === "possibleMoves") {
        ...
        if (
          move.includes("Castle") &&
          !this.isCastlingLegal(board, kingColor, move)
        ) {
          continue;
        }
        squares.push(possibleSquare);
  ...
  areCastlingSquaresControlled(board, kingColor, castlingDirection) {
    ...
    return unsafeSquares.some((unsafeSquare) =>
  ...
  isMoveLegal(board, move) {
    const legalMoves = this.findSquaresForPiece(
      board,
      move.from,
      "possibleMoves"
    );
  ...
  getFullMove(board, move) {
    ...
    if (
      this.isKingInCheckMate(board, this.getOpposingColor(move.piece.color))
    ) {
  ...
  getSan(move) {
    console.log("getting san for move:", move);
    let san = "";
  ...
  getPieceLetter(piece, use) {
    ...
    return color === "black" && use === "console"
      ? pieceLetters[type].toLowerCase()
      : pieceLetters[type];
  }
